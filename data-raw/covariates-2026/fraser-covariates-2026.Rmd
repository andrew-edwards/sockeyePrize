...
end_year <- 2025              # Think this will be the year before the current
# year, have to check (it was automated from specific salmon data in 2025 code)
```

TODO Had `Analysis for Fraser River Chilko` etc. (see `analysis-2025-3.Rmd` but removing here to just focus
...
`max(sub.dat$year)` but didn't work, ignoring).
...
plot(halibut_bank_spring$year,
     ...
     ylab = "Year")
```
So no `r end_year` `sst_spring`, which is fine as should not be impacting `r end_year` returns.
