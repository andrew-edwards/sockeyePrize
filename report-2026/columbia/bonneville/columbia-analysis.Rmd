---
title: "Columbia River 2026 - single stock analysis"
...
package. Doing Columbia first as it's the only one we currently have data for,
and to get it as a template for others.
...
Try to predict the following (data are saved in `returns_columbia_2026`):
...
```{r, returns}
returns_columbia_2026
summary(returns_columbia_2026)
```
...
river_returns <-
  dplyr::filter(returns_columbia_2026,
                River == river) %>%
