...
We want to predict `returns_fraser_2026ll$Total_Returns`, by doing age4 and age5
...
HERE - seems okay so far. Need to figure out covariates to use, doing the
correlation stuff. Have to think a little. Run the .rmd

TODO move the envtl covriates explanation section up (this current part is
modified from something else).
...
- Chilko: `total_spawners` or `effective_spawners` with either `pink` or `total`; though
  ...
  with `effective_spawners` and `pink` though. Late Stuart: not correlated now
  (so bit more justified in sticking with them). Very cyclic compared to Chilko.

So, as for Chilko, remove:
...
First, lags with PDO (not BI). Can take a while so saving the results as .rds
...
So results in 19 lags below, which is about two hours.
...
TODO Can delete this onwards I think, was for Columbia 2026 but FR is quite different.
