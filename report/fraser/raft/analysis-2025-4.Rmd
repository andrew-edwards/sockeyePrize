...
For Quesnel, non-stock specific covariates:
...
- Chilko: `total_spawners` or `effective_spawners` with either `pink` or `total`; though
  ...
  with `effective_spawners` and `pink` though. Late Stuart and Quesnel: not correlated now
  (so bit more justified in sticking with them). Very cyclic compared to Chilko.

So, as for Chilko and Late Stuart, remove:
...
lags_reduced_pdo_not_in_top_20 <- list(pdo_winter_mean = c(1),
                                       zoo_total_biomass = c(1, 2))
```

Chilko had `fraser_mean_spring` lag 1, Late Stuart did not (so took twice as
long to run the age5 calculation).
...
So results in 18 lags below, which is about one hour.
