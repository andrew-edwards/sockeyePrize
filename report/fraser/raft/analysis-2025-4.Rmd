...
takes a while and previous chunks have changed. Not running as maybe too big to
cache, but .rds was saved.

For Raft age5 that crashed computer, since .rds file is 317Mb, plus tried to
cache also, plus I hadn't turned computer off for a week probably. Aha - it
saved the .rds file, but didn't do the big cache of results. Also don't cache
the next chunks, since .rds is saved anyway (means the subsequent chunks might
always get rerun, but they're not big calculations so that's fine).
