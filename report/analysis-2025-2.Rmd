...
See `analysis-2025-2.Rmd` instead, which copies and then changes this. See note
on page 77 as to why (basically need to not temporally shift covariates for the input, but
let EDM deal with the lags).
...
Next:
  ...
  D do correlation plot at start of EDM with covariates section
...
MAYBE don't do the shifting, just leave that (i.e. the lags) to EDM. EDM might
be doing the shift anyway. And could then maybe take out the -1 I put into
`pbsEDM::untransform_predictions()`. YES - let EDM do the shifting, and just
select sensible lags based on the biology. See our paper Appendix.
