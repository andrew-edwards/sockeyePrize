---
title: "Analysing data for 2025 competition - not manually lagging covariates"
...
knitr::opts_chunk$set(
  ...
  comment = "",
  cache = TRUE,
  cache_path = "analysis-2025-2-cache/",
  fig.path = "analysis-2025-2-figs-cache/",
  fig.width = 7,
  fig.height = 6
)
...
rmarkdown::render("analysis-2025-2.Rmd")
```
...
Adapting from `analysis-2025-1.Rmd` in which we manually lagged covariates to
all have the same `return_year`. But this messes up the EDM projections, so
redoing here without lagging manually, and let EDM select the lags (or we can
prescribe them). Removing some extra notes from here, but keeping this self
contained (keeping the descriptions and data wrangling, which we need to change
anyway).
...
Next:
 ...
 - take out the manual lagging to give `return_year`
 - figure out lags to to use (or just do all) for multivariate EDM
 - may need to look at multiview embedding code, might want something similar to loop
 ...
 - And could maybe try multiview also but less polished in the code. Though a
    previous attempt seemed to work quite well (actual results weren't great,
    but was because I had the lags already in there which created a
    lot of NA's).
...
```{r fullspan}
cov_in_first_year <- which(!is.na(input_age4[1, ]))
...
```{r, fullspan2}
input_age4_full_span <- input_age4[ , cov_in_first_year]
summary(input_age4_full_span)    # Just the one NA for final return_year of age4

GGally::ggpairs(input_age4_full_span)
```

A clearer plot (coefficients are calculated slightly differently):
```{r, fullspan3}
corrplot::corrplot(cor(filter(input_age4_full_span, return_year < max(input_age4_full_span$return_year))),
  method = "number",
  type = "upper" # show only upper side
  )
```

Can see there's barely any correlation between any covariates and age4's.
...
PDO and either Fraser Discharge (unfortunate)

- `pdo_winter_mean` and either `fraser_mean_spring` or `fraser_peak_spring`; the
  latter are direct influences so should probabarly be preferred.
...
```{r edmspan1}
res_span1 <- pbsEDM(input_age4_full_span,
...
So, let's use that, and think about the lags. Can define the lags here. Since it
ignores other covariates, we don't need to create subsets of the data when
removing a correlated covariate.

```{r, mvespan1}
lags_span1 <- list(age4 = c(0, 4, 5),
                   # pdo_winter_mean = 0,
                   fraser_mean_spring = 0,
                   pink = 0)

mve_span1 <- multiview_embedding(data = input_age4_full_span,
                                 response = "age4",
                                 lags = lags_span1)  # Do lags_use_multi
...
D - All looked good until that last one full of NA's. Need to think a bit more -
reread Appendix and think about what we need.

D May have the indexing wrong, and need to have final age4 have a value in the
column. Think that NA is getting used, hence giving NA's in the predictions.

MAYBE don't do the shifting, just leave that (i.e. the lags) to EDM. EDM is
doing the shift anyway. And could then maybe take out the -1 I put into
`pbsEDM::untransform_predictions()`.

YES - let EDM do the shifting, and just select sensible lags based on the
biology. See our paper Appendix.

DOING this in analysis-2025-1.Rmd, copying this file over (want to compare results).
