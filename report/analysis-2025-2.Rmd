...
Next:
 ...
 D do a big run over lunch, see span5 and then span6. span6 is best so far.

 - get the spawners in also (will need wrangling for other stocks anyway). Do
   this analysis with spawners and use that if it's better, else stick with
   this. Then move onto other stocks.

 D understand E definition, regarding univariate - yes, $E = 5$ is a lag of 4,
    since it counts the lag of 0.

 D take out the manual lagging to give `return_year`; so just have 'year'.

 D may need to look at multiview embedding code, might want something similar to loop
 ...
 - Just going to do multiview (but less polished in the code).

 - figure out lags to to use (or just do all) for multivariate EDM
 - Can always try best multiview lags in multivariate after the lags have been 'optimised'.
...
T <- nrow(input_age4)          # T in EDM speak, final index year of data.
T
```

So only NA's in `sst_spring`, `zoo_total_biomass` and `bi_anomaly`, and only zoo
is in the tail.
...
the only one). Also, can't have lag of 1 for zooplankton.
...
summary(input_age4_span)    # No NA's at all now (as have removed 2025)
input_age4_span %>% tail() %>% a()

GGally::ggpairs(input_age4_span)
```

Strange that coefficients have changed a bit from version 1 (all we've done is
...
This didn't actually take too long to run, so probably could be okay with just
cacheing, but already saved so using that, and not evaluating next chunk.rerunning but just showing the start of the results.
...
Can actually see that top one here was the 10th one in span5:
```{r comparespan56}
...
**Just try span6 best model as multivariate EDM**
...
So far span6 is best, has highest $\rho$ and does not have age4 (or anything)
with lag of 0, which we cannot use for prediction.

Interesting that all the predictions seem low, though maybe they're extracting
