...
- As for Fraser River, temperature from buoys: "West Sea Otter" (Apr-Jul average), "East Dellwood Knolls" (Apr-Jul average), "Halibut Bank" (Apr-Jun average). Year of outmigration, after the PDO Mar calculation.
...
Now for PDO. Bit fiddly to group Nov to Mar, doing a quick kludge here but
should generalise in a function and add some checks.
...
```
That suggests $E=5$ when using no covariates, and the prediction for 2025 would
be the last (CHECK) of these:
```{r pred1}
...
Now try some covariates, need to understand what is going on here.

```{r edm1}
res <- pbsEDM(input_age4,
              lags = list(age4 = c(0, 4),
                          at_sea_abundance = 0,
                          pdo_winter_mean = 0),
