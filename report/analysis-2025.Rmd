...
Construct `input_age4` tibble to go into pbsEDM, based on ideas above. `return_year` is the year that
...
at_sea_abundance <- filter(at_sea_all,
                           River == river) %>%
  select(marine_entry_year = MarineEntryYear,
         total_abundance = Juveniles_Marine_Entry) %>%
  mutate(return_year = marine_entry_year + 2)

at_sea_abundance

at_sea_abundance$total_abundance     # visually check that only early and end ones
                                     # are 0, for next line

at_sea_abundance <- filter(at_sea_abundance,
                           total_abundance > 0)
